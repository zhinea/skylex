...
func TestNodeService_DrainUnassignedNodeKeepsNodeRegistered(t *testing.T) {
	database, log := newTestDeps(t)
	conn := database.Conn()
	nodes := db.NewNodeRepository(conn, log)
	clusters := db.NewClusterRepository(conn, log)
	...
	// Unassigned node (no cluster) with a linked agent.
	...
	// Drain must NOT delete the node; the agent keeps a valid identity so it
	// does not fall into an "unknown agent" retry loop.
	updated, err := nodes.GetByID(context.Background(), node.ID)
	if err != nil {
		t.Fatalf("get node: %v", err)
	}
	if updated == nil {
		t.Fatal("expected unassigned node to remain registered after drain")
	}
	if updated.Status != models.NodeStatusDrained {
		t.Fatalf("expected drained status, got %q", updated.Status)
	}
